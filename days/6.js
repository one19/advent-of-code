...
const deltas = (x, y, size) => {
  ...
  Array.from({ length: size }, (_, i) => i).forEach(deltaX => {
    Array.from({ length: size }, (_, i) => i).forEach(deltaY => {
      if (deltaX + deltaY !== size - 1) return;
...
const grow = (coordinates, growthDelta = 2) => {
  ...
  if (!nulledCoords.length) return matrix;
